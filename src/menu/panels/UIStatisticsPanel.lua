local Statistics = {
    Background = { Dictionary = "commonmenu", Texture = "gradient_bgd", X = 431, Y = 50, Width = 431, Height = 42 },
    Text = { -- 618
        Left = { X = 431, Y = 55, Scale = 0.35 },
    },
    Bar = { Right = 40, Y = 67, Width = 200, Height = 10, OffsetRatio = 0.5 },
    Divider = {
        [1] = { X = 200, Y = 67, Width = 2, Height = 10 },
        [2] = { X = 200, Y = 67, Width = 2, Height = 10 },
        [3] = { X = 200, Y = 67, Width = 2, Height = 10 },
        [4] = { X = 200, Y = 67, Width = 2, Height = 10 },
        [5] = { X = 200, Y = 67, Width = 2, Height = 10 },
    }
}
...
function RageUI.StatisticPanel(Percent, Text, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            RenderRectangle(Statistics.Background.X, Statistics.Background.Y + (RageUI.StatisticPanelCount * 42), Statistics.Background.Width + CurrentMenu.WidthOffset, Statistics.Background.Height, 0, 0, 0, 122)
            RenderText(Text or "", Statistics.Background.X + 8.0, (RageUI.StatisticPanelCount * 40) +  Statistics.Text.Left.Y, 0, Statistics.Text.Left.Scale, 245, 245, 245, 255, 0)
            
            RenderRectangle(CurrentMenu.X + Statistics.Background.X + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right + CurrentMenu.WidthOffset, (RageUI.StatisticPanelCount * 40) + Statistics.Bar.Y, BarWidth, Statistics.Bar.Height, 87, 87, 87, 255)
            RenderRectangle(CurrentMenu.X + Statistics.Background.X + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right + CurrentMenu.WidthOffset, (RageUI.StatisticPanelCount * 40) + Statistics.Bar.Y, Percent * BarWidth, Statistics.Bar.Height, 255, 255, 255, 255)
            for i = 1, #Statistics.Divider, 1 do
                RenderRectangle((CurrentMenu.X + Statistics.Background.X + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right) + i * ((BarWidth - (#Statistics.Divider / Statistics.Divider[i].Width)) / (#Statistics.Divider + 1)) + CurrentMenu.WidthOffset, (RageUI.StatisticPanelCount * 40) + Statistics.Divider[i].Y, Statistics.Divider[i].Width, Statistics.Divider[i].Height, 0, 0, 0, 255)
            end
